...
export default function AlertLogsPage() {
  ...
  const { data: alertsResponse, isLoading, mutate } = useSWR(
    apiUrl,
    ([url]) => globalFetcher(url, session?.accessToken),
    { revalidateOnFocus: false }
  );

  const alerts = useMemo(() => alertsResponse?.data?.data || [], [alertsResponse]);
  const totalItems = alertsResponse?.data?.total || 0;
  const totalPages = alertsResponse?.data?.last_page || 1;
  ...
  useGSAP(() => {
    if (selectedAlert && drawerRef.current) {
  ...
  const handleCloseDrawer = () => {
    const tl = gsap.timeline({ onComplete: () => { setSelectedAlert(null); setReplyText(""); } });
    tl.to(drawerRef.current, { x: "100%", duration: 0.3, ease: "power3.in" });
    tl.to(overlayRef.current, { opacity: 0, duration: 0.2 }, "-=0.1");
  };

  const handleUpdateStatus = async (alertId, isApproved) => {
    setIsUpdatingStatus(true);
    // Optimistically determine the next status
    const nextApprovedStatus = !isApproved;
    try {
      await globalFetcher(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/reviews/${alertId}/toggle-status`,
        session?.accessToken,
        { method: "PATCH", headers: { "Content-Type": "application/json" } }
      );
      toast.success(`Alert status toggled successfully`);
      mutate();
      if (selectedAlert?.id === alertId) {
        setSelectedAlert((prev) => ({ ...prev, is_approved: nextApprovedStatus }));
      }
    } catch (err) {
  ...
  const handleReply = async () => {
    ...
    try {
      await globalFetcher(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/reviews/${selectedAlert.id}/reply`,
        session?.accessToken,
      ...
      setReplyText("");
      mutate();
      setSelectedAlert((prev) => ({ ...prev, admin_reply: replyText }));
    } catch (err) {
  ...
  const handleExport = () => {
    ...
    exportToCSV(alerts, "Alert Logs", headerMap);
  };
    ...
    >
        ...
        <div className="animate-header">
          <div className="flex gap-4 items-stretch">
            ...
            <div className="flex flex-col justify-center py-1">
              <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">Alert Logs</h1>
              <p className="text-sm text-slate-500 dark:text-slate-400 font-medium mt-1">Manage customer product alerts and ratings.</p>
            </div>
          </div>
        </div>
        <div className="animate-header">
      ...
      <div className="animate-header grid grid-cols-1 sm:grid-cols-3 gap-5">
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Total Alert Logs</p>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Pending Alert</p>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50/80 dark:bg-slate-900/80 border-b border-slate-200 dark:border-slate-700">
              <tr>
                ...
                <th className="p-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Alert</th>
                ...
                alerts.map((alert) => (
                  ...
                  >
                    ...
                    <td className="p-4 max-w-[180px]">
                      ...
                      {alert.variant?.name && (
                        <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5 truncate">{alert.variant.name}</p>
                      )}
                    </td>
                    {/* Rating */}
                    <td className="p-4">
                      <RatingBadge rating={alert.rating} />
                    </td>
                    {/* Alert Snippet */}
                    <td className="p-4 max-w-[260px]">
                      {alert.title && (
                        <p className="text-xs font-semibold text-slate-800 dark:text-slate-200 truncate mb-0.5">{alert.title}</p>
                      )}
                    ...
                    {/* Status */}
                    <td className="p-4">
                      <StatusBadge isApproved={alert.is_approved} />
                    </td>
                    {/* Date */}
                    ...
                    <td className="p-4 pr-6 text-right">
                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={(e) => { e.stopPropagation(); handleUpdateStatus(alert.id, alert.is_approved); }}
                          className={`p-1.5 rounded-lg transition-colors ${alert.is_approved ? "text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-900/30" : "text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/30"}`}
                          title={alert.is_approved ? "Reject" : "Approve"}
                        >
                          {alert.is_approved ? <XCircle className="w-4 h-4" /> : <CheckCircle2 className="w-4 h-4" />}
                        </button>
                ...
                <tr>
                  <td colSpan="7" className="text-center py-20">
                    <div className="flex flex-col items-center justify-center text-slate-400">
                      ...
                      <p className="text-sm font-medium text-slate-400 dark:text-slate-500">No alerts found.</p>
            ...
            >
              ...
              <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex items-start justify-between bg-white dark:bg-slate-900 sticky top-0 z-10">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <StarDisplay rating={selectedAlert.rating} size="lg" />
                    <StatusBadge isApproved={selectedAlert.is_approved} />
                  </div>
                  <p className="text-xs text-slate-400 dark:text-slate-500">
                    {new Date(selectedAlert.created_at).toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" })}
                  </p>
                </div>
                <button
                  onClick={handleCloseDrawer}
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50/30 dark:bg-slate-950/30">
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5">
                  ...
                  <div className="flex items-center gap-3">
                    <img src={getAvatarUrl(selectedAlert.user)} className="w-12 h-12 rounded-xl object-cover border border-slate-100 dark:border-slate-700" alt="" />
                    <div>
                      <p className="font-bold text-slate-900 dark:text-white">{selectedAlert.user?.name || "Anonymous"}</p>
                      <p className="text-xs text-slate-400 dark:text-slate-500">{selectedAlert.user?.email || "—"}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5">
                  ...
                  <div className="flex items-center gap-3">
                    {selectedAlert.product?.primary_image_path ? (
                      <img
                        src={`${process.env.NEXT_PUBLIC_API_BASE_URL?.replace("/api/v1", "")}/${selectedAlert.product.primary_image_path}`}
                    ...
                    <div>
                      <p className="font-bold text-slate-900 dark:text-white text-sm">{selectedAlert.product?.name || selectedAlert.product_name || "—"}</p>
                      {selectedAlert.variant?.name && (
                        <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{selectedAlert.variant.name}</p>
                      )}
                    </div>
                  </div>
                </div>
                ...
                {/* Alert Content */}
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5">
                  <h3 className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-3">Alert</h3>
                  {selectedAlert.title && (
                    <p className="font-bold text-slate-900 dark:text-white text-base mb-2">{selectedAlert.title}</p>
                  )}
                  <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
                    {selectedAlert.alert || selectedAlert.body || "No written alert."}
                  </p>
                </div>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5">
                  ...
                  {selectedAlert.admin_reply ? (
                    <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800/50 rounded-xl p-4">
                      <p className="text-sm text-indigo-800 dark:text-indigo-300 leading-relaxed">{selectedAlert.admin_reply}</p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      <textarea
                        ...
                        placeholder="Write a reply to this alert..."
                        rows={3}
              ...
              <div className="p-5 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 flex justify-end gap-3">
                <button
                  onClick={() => handleUpdateStatus(selectedAlert.id, selectedAlert.is_approved)}
                  disabled={isUpdatingStatus}
                  className={`flex items-center gap-2 px-5 py-2.5 text-sm font-bold text-white rounded-xl shadow-lg transition-all active:scale-95 disabled:opacity-50 ${selectedAlert.is_approved ? "bg-rose-600 hover:bg-rose-700 shadow-rose-600/20" : "bg-emerald-600 hover:bg-emerald-700 shadow-emerald-600/20"}`}
                >
                  {isUpdatingStatus ? <Loader2 className="w-4 h-4 animate-spin" /> : (selectedAlert.is_approved ? <ThumbsDown className="w-4 h-4" /> : <ThumbsUp className="w-4 h-4" />)}
                  {selectedAlert.is_approved ? "Reject" : "Approve"}
                </button>
              </div>
            </div>
          </div>
