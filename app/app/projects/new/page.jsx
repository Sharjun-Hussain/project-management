...
const ProjectRelationshipSelector = ({
  title,
  description,
  selectedIds,
  onUpdate,
  ...
  onSearchChange,
  results,
  projectDetailsMap, // Add map of project details
}) => {
    ...
    <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
      ...
      <div className="space-y-6">
                ...
                results.map((project) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 overflow-hidden">
                        {project.primary_image_path ? (
                          <img
                            src={`${process.env.NEXT_PUBLIC_API_BASE_URL?.replace("/api/v1", "")}/${project.primary_image_path}`}
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${selectedIds.includes(project.id)
                          ? "bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 hover:bg-red-100"
                          : "bg-indigo-600 text-white hover:bg-indigo-700 shadow-md shadow-indigo-600/20"
                        }`}
                    >
                      {selectedIds.includes(project.id) ? "Remove" : "Add"}
                    </button>
                  </div>
                ))
              )}
        ...
        <div className="space-y-3">
          <p className="text-xs font-medium text-slate-400">
            Selected Projects ({selectedIds.length})
          </p>
          {selectedIds.length === 0 ? (
            <div className="p-8 text-center bg-slate-50 dark:bg-slate-900 rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-700">
              ...
              <p className="text-slate-400 text-xs">No projects selected yet.</p>
            </div>
          ) : (
...
function CreateProjectContent() {
  ...
  const DRAFT_KEY = React.useMemo(() =>
    session?.user?.id ? `igen_temp_project_create_draft_${session.user.id}` : "igen_temp_project_create_draft",
    [session?.user?.id]
  );
  ...
  const [projectSetupType, setProjectSetupType] = useState(null); // 'direct' | 'variant' | null
  ...
  const handleSwitchSetupType = () => {
    if (window.confirm("Are you sure you want to switch setup types? Switching will reset your current variant configurations.")) {
      if (projectSetupType === "direct") {
        setProjectSetupType("variant");
        setVariants([]);
      } else {
        setProjectSetupType("direct");
        setVariants([
          {
            variant_name: "Default",
            sku: "",
  ...
  const completionPercentage = React.useMemo(() => {
    ...
    if (projectImages.some(img => img.isPrimary)) score++;
  ...
  }, [formData, projectImages, isEditMode, selectedFeatures, variants, isPhoneCategory]);
  ...
  const isStepComplete = (stepId) => {
    switch (stepId) {
      ...
      case "media":
        return projectImages.some(img => img.isPrimary);
      case "variants":
  ...
  // Project Relationship Search State
  const [projectSearchTerm, setProjectSearchTerm] = useState("");
  const [debouncedProjectSearch, setDebouncedProjectSearch] = useState("");
  const [projectSearchResults, setProjectSearchResults] = useState([]);
  const [isSearchingProjects, setIsSearchingProjects] = useState(false);
  const [projectDetailsMap, setProjectDetailsMap] = useState({}); // Stores details for all fetched/selected projects
  ...
  // Fetch projects for relationship search
  const { data: searchResponse } = useSWR(
    session?.accessToken && debouncedProjectSearch.length > 2
      ? [
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/products?search=${debouncedProjectSearch}`,
        session.accessToken,
      ]
      : null,
    ([url]) => globalFetcher(url, session?.accessToken)
    ...
    if (draftId) {
      try {
        const drafts = JSON.parse(
          localStorage.getItem("igen_project_drafts") || "[]",
        );
      ...
      const fetchProject = async () => {
        try {
          const data = await globalFetcher(
            `${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/products/${projectId}`,
            session.accessToken
          );
          ...
          setFormData({
            ...
            type: project.type || "physical",
            status: project.is_active ? "published" : "draft",
            short_description: project.short_description || "",
            full_description: project.full_description || "",
            is_featured: project.is_featured || false,
            is_trending: project.is_trending || false,
            is_new_arrival: project.is_new_arrival || false,
            is_active: project.is_active || false,
            bundled_project_ids: project.bundled_projects?.map(p => p.id) || [],
            compatible_project_ids: project.compatible_projects?.map(p => p.id) || [],
          });

          // Load variants
          if (project.variants && project.variants.length > 0) {
            setVariants(project.variants.map(v => ({
              ...v,
            ...
            if (project.variants[0]?.condition) {
              setFormData(prev => ({
                ...prev,
                condition: project.variants[0].condition
              }));
            }
          } else {
            setProjectSetupType("direct");
          ...
          const loadedImages = [];
          if (project.primary_image_path) {
            loadedImages.push({
              id: `primary-${Date.now()}`,
              file: null,
              previewUrl: getImageUrl(project.primary_image_path),
              isPrimary: true,
              isExisting: true,
              existingUrl: project.primary_image_path
            });
          }
          ...
          if (project.bundled_projects) {
            project.bundled_projects.forEach(p => existingDetails[p.id] = p);
          }
          if (project.compatible_projects) {
            project.compatible_projects.forEach(p => existingDetails[p.id] = p);
          }
  ...
  const buildFormData = () => {
    ...
    const primaryImg = projectImages.find(img => img.isPrimary);
    if (primaryImg) {
      if (primaryImg.file) {
    ...
    const newGalleryFiles = projectImages.filter(img => !img.isPrimary && img.file);
    newGalleryFiles.forEach((img) => {
      data.append("images[]", img.file);
    });

    // Existing gallery images retained
    const existingGalleryPaths = projectImages.filter(img => !img.isPrimary && img.isExisting).map(img => img.existingUrl);
    ...
    if (formData.bundled_project_ids.length > 0) {
      data.append("bundled_project_ids", formData.bundled_project_ids.join(","));
    }
    if (formData.compatible_project_ids.length > 0) {
      data.append("compatible_project_ids", formData.compatible_project_ids.join(","));
    }
  ...
  const handleSave = async () => {
    ...
    if (!formData.name) validationList.push({ stepId: "general", stepLabel: "General Info", field: "name", message: "Project Name is required" });
    if (!formData.category_id) validationList.push({ stepId: "general", stepLabel: "General Info", field: "category_id", message: "Project Category is required" });
    if (!formData.type) validationList.push({ stepId: "general", stepLabel: "General Info", field: "type", message: "Project Type is required" });

    if (!projectImages.some(img => img.isPrimary)) {
    ...
    if (variants.length === 0) {
      ...
      variants.forEach((v, idx) => {
        const prefix = projectSetupType === "direct" ? "Project" : `Variant "${v.variant_name || 'Default'}"`;
  ...
  if (!projectSetupType && !isEditMode) {
    return (
        ...
        <div className="max-w-md w-full bg-white dark:bg-slate-900/90 backdrop-blur-md rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-2xl dark:shadow-slate-950/50 animate-in fade-in zoom-in-95 duration-200">
          <div className="text-center space-y-1 mb-6">
            <h1 className="text-base font-extrabold text-slate-900 dark:text-white tracking-tight">
              Select Project Setup Type
            </h1>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Choose the catalog configuration for your project.
            </p>
          </div>

          <div className="space-y-3">
            {/* Option 1: Direct Project */}
            <button
              type="button"
              onClick={() => {
                setProjectSetupType("direct");
    ...
    >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-8 pt-4 sm:pt-6 pb-12 transition-all">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 lg:gap-8">
          ...
          <div className="md:col-span-8 lg:col-span-9 space-y-6 lg:space-y-8 min-w-0">
                  ...
                  <div className="space-y-4">
                    ...
                    <FormInput
                      ...
                      onChange={(e) => {
                        ...
                        if (projectSetupType === "direct" && !isSkuManuallyEdited) {
                  ...
                  <div className="flex justify-between items-center mb-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                        Upload your project images and choose which one is the main display photo.
                      </p>
                    </div>
                  </div>
                  ...
                  {projectImages.length > 0 && (
                    <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-800">
                      <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-4 flex items-center gap-2">
                        Uploaded Photos ({projectImages.length})
            ...
            {activeTab === "related" && (
              <div className="space-y-6 animate-fade-up">
                <ProjectRelationshipSelector
                  title="Related Projects"
                  description="Select projects that are similar or compatible with this one."
                  selectedIds={formData.compatible_project_ids}
                  onUpdate={(ids) =>
                    setFormData({ ...formData, compatible_project_ids: ids })
                  }
                  searchTerm={projectSearchTerm}
                  onSearchChange={setProjectSearchTerm}
                  results={projectSearchResults}
                  projectDetailsMap={projectDetailsMap}
                />
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 pb-4 border-b border-slate-100 dark:border-slate-700">
                    <div>
                      <h3 className="text-lg font-semibold text-slate-800 dark:text-white">
                        {projectSetupType === "direct" ? "Project Pricing & Inventory" : "Manage Variants"} <span className="text-red-500">*</span>
                      </h3>
                      <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">
                        {projectSetupType === "direct"
                          ? "Configure the pricing, inventory stock, and project properties."
                          : "Configure multiple variations (sizes, colors, RAM/storage size, etc.) for this project."}
                      </p>
                    </div>
                    <div className="shrink-0 bg-slate-100 dark:bg-slate-900 p-1 rounded-xl flex items-center border border-slate-200/50 dark:border-slate-800">
                      <button
                        ...
                        onClick={() => {
                          if (projectSetupType !== "direct") {
                            if (window.confirm("Are you sure you want to switch to Direct Project? This will reset custom variants.")) {
                              setProjectSetupType("direct");
                              setVariants([
                                {
                                  variant_name: "Default",
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all duration-200 cursor-pointer ${projectSetupType === "direct"
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          if (projectSetupType !== "variant") {
                            if (window.confirm("Are you sure you want to switch to Variant Project? This will reset direct pricing properties.")) {
                              setProjectSetupType("variant");
                              setVariants([]);
                            }
                          }
                        }}
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all duration-200 cursor-pointer ${projectSetupType === "variant"
                            ? "bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 shadow-sm border border-slate-200/10"
                            : "text-slate-500 hover:text-slate-700 dark:hover:text-slate-300"
                          }`}
                      >
      ...
      <div className="fixed bottom-4 left-6 right-6 z-[100] flex items-center justify-between pointer-events-none">
        ...
        <div className="pointer-events-auto flex items-center gap-2 px-4 py-2 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border border-slate-200 dark:border-slate-700 rounded-full shadow-xl text-[10px] font-medium text-slate-500 dark:text-slate-400 transition-all duration-300">
          ...
          <span className="shrink-0">IMAGES: <span className="text-slate-900 dark:text-white font-semibold">{projectImages.length}</span></span>
          <span className="text-slate-300 dark:text-slate-600 mx-1">|</span>
          ...
          <div className="max-w-md w-full bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 max-h-[60vh] overflow-y-auto space-y-4">
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                To publish this project, please provide the following required details. Click any item below to jump directly to that field:
              </p>
              <div className="space-y-2">
